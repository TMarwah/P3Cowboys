...
@Cowboys_Marc_bp.route("/minilab", methods=["POST", "GET"])
def minilab():
    if(request.method == 'POST'):
        ...
        return render_template("minilab.html", Robot=Robot(), robots=robots,sum=sum,diff=diff, product=product,quotient=quotient, nums=nums)
    ...
    robots.append(Robot1("Suzan", 50, "Orange"))
    robots.append(Robot1("Bob", 100, "Baby Blue"))
    return render_template("minilab.html", Robot=Robot(), robots=robots)
